...
const PRICE_CONFIG = {
  'Продать': {
    'VC': { min: 200000, max: 20000000000, period: null },
    '$': { min: 2000000, max: 600000000000, period: null },
    'BTC': { min: 200, max: 6000000, period: null },
    'EURO': { min: 20000, max: 260000000, period: null }
  },
  'Купить': {
    'VC': { min: 200000, max: 20000000000, period: null },
    '$': { min: 2000000, max: 600000000000, period: null },
  ...
  'Сдать в аренду': {
    'VC': { min: 100000, max: 10000000, period: 'час' },
    '$': { min: 1000000, max: 600000000, period: 'час' },
    'BTC': { min: 100, max: 60000, period: 'час' },
    'EURO': { min: 10000, max: 400000, period: 'час' }
  }
};
...
function Vehicle() {
  ...
  useEffect(() => {
    document.title = 'Arz Store | Транспорт';
  }, []);
  ...
  const fetchAds = async (currentOffset) => {
    ...
    try {
      ...
      let url = `http://localhost:8080/api/ads?category=vehicles&server=${selectedServer}&limit=20&offset=${currentOffset}`;
  ...
  const fetchAdCount = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/getadcount?CategoryName=vehicles');
      const data = await response.json();
  ...
  const handleCreateListing = async () => {
    ...
    const imagePath = `ads/vehicle/${timestamp}_${user?.nickname || 'user'}.${fileExtension}`;
    ...
    formDataToSend.append('category', 'vehicles');
          ...
          <div className="listing-form">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={handleInputChange}
                maxLength={25}
                placeholder="Например: Lamborghini Huracan"
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={handleInputChange}
                maxLength={500}
                placeholder="Опишите транспорт подробнее..."
    ...
    <div className="mobile-category-page">
      ...
      <div className="mobile-category-content">
        <div className="mobile-category-header">
          ...
          <h1 className="mobile-category-title">Транспорт</h1>
        </div>
