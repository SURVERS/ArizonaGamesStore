...
const LISTING_TYPES = ['Продать', 'Купить', 'Сдать в аренду'];
const PRICE_CONFIG = {
  'Продать': {
    'VC': { min: 10000, max: 5000000000, period: null },
    '$': { min: 100000, max: 150000000000, period: null },
    'BTC': { min: 10, max: 1500000, period: null },
    'EURO': { min: 1000, max: 65000000, period: null }
  },
  'Купить': {
    'VC': { min: 10000, max: 5000000000, period: null },
    '$': { min: 100000, max: 150000000000, period: null },
  ...
  'Сдать в аренду': {
    'VC': { min: 5000, max: 2500000, period: 'час' },
    '$': { min: 50000, max: 150000000, period: 'час' },
    'BTC': { min: 5, max: 15000, period: 'час' },
    'EURO': { min: 500, max: 100000, period: 'час' }
  }
};
...
function Accs() {
  ...
  useEffect(() => {
    document.title = 'Arz Store | Аксессуары';
  }, []);
  ...
  const fetchAds = async (currentOffset) => {
    ...
    try {
      let url = `http://localhost:8080/api/ads?category=accs&server=${selectedServer}&limit=20&offset=${currentOffset}`;
  ...
  const fetchAdCount = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/getadcount?CategoryName=accs');
      const data = await response.json();
  ...
  const handleOpenReportMenu = (ad, e) => {
    e.stopPropagation();
    setShowReportMenu(showReportMenu === ad.id ? null : ad.id);
  };

  const handleOpenReport = (ad) => {
  ...
  const handleCloseReport = () => {
    setReportModal({ isOpen: false, ad: null });
    setReportReason('');
    setReportDescription('');
  };
  ...
  const handleSubmitReport = async () => {
    if (!reportReason) {
      setToast({ message: 'Выберите причину жалобы', type: 'error' });
      return;
    }

    if (reportReason === 'Другое' && (!reportDescription || reportDescription.length < 10)) {
      setToast({ message: 'Опишите проблему (минимум 10 символов)', type: 'error' });
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:8080/api/reports', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          ad_id: reportModal.ad.id,
          reason: reportReason,
          description: reportReason === 'Другое' ? reportDescription : null
        })
      });

      if (response.ok) {
        setToast({ message: '✓ Жалоба успешно отправлена! Спасибо за обращение, мы рассмотрим её в ближайшее время.', type: 'success' });
        handleCloseReport();
      } else {
        const data = await response.json();
        setToast({ message: data.error || 'Ошибка отправки жалобы', type: 'error' });
      }
    } catch (error) {
  ...
  const handleCreateListing = async () => {
    ...
    if (formData.type === 'Сдать в аренду') {
      if (!formData.rentalHoursLimit || formData.rentalHoursLimit === '0') {
        setToast({ message: 'Укажите лимит часов аренды (от 1 до 180)', type: 'error' });
        return;
      }
    ...
    const imagePath = `ads/accs/${timestamp}_${user?.nickname || 'user'}.${fileExtension}`;
    ...
    formDataToSend.append('category', 'accs');
    formDataToSend.append('nickname', user?.nickname || 'Unknown');

    if (formData.type === 'Сдать в аренду' && formData.rentalHoursLimit) {
          ...
          <div className="listing-form">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={handleInputChange}
                maxLength={25}
                placeholder="Например: Очки Supreme"
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={handleInputChange}
                maxLength={500}
                placeholder="Опишите аксессуар подробнее..."
            ...
            {formData.type === 'Сдать в аренду' && (
              <div className="form-group">
                <label>Лимит часов аренды * (от 1 до 180 часов)</label>
                <input
                  type="text"
    ...
    <div className="mobile-category-page">
      ...
      <div className="mobile-category-content">
        <div className="mobile-category-header">
          ...
          <h1 className="mobile-category-title">Аксессуары</h1>
        </div>
              ...
              {listings.map((ad) => (
                <div key={ad.id} className="mobile-listing-card">
                  <div className="mobile-listing-image">
                    ...
                    <button
                      className="mobile-listing-menu-btn"
                      onClick={(e) => handleOpenReportMenu(ad, e)}
                    >
                      ⋮
                    </button>
                    {showReportMenu === ad.id && (
                      <div className="mobile-listing-menu">
                        <button onClick={() => handleOpenReport(ad)}>
                          🚩 Пожаловаться
                        </button>
                      </div>
                    )}
      ...
      {reportModal.isOpen && (
        <div className="modal-overlay" onClick={handleCloseReport}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h2>ПОДАЧА ЖАЛОБЫ НА ОБЪЯВЛЕНИЕ</h2>
            <div className="report-ad-preview">
              <img src={reportModal.ad.image} alt={reportModal.ad.title} />
              <div>
                <p><strong>ID:</strong> {reportModal.ad.id}</p>
                <p><strong>Название:</strong> {reportModal.ad.title}</p>
                <p><strong>Автор:</strong> @{reportModal.ad.nickname}</p>
              </div>
            </div>
            <div className="report-reasons">
              <button
                className={reportReason === 'Жестокое обращение с детьми' ? 'active' : ''}
                onClick={() => setReportReason('Жестокое обращение с детьми')}
              >
                Жестокое обращение с детьми
              </button>
              <button
                className={reportReason === 'Насилие' ? 'active' : ''}
                onClick={() => setReportReason('Насилие')}
              >
                Насилие
              </button>
              <button
                className={reportReason === 'Незаконные товары и услуги' ? 'active' : ''}
                onClick={() => setReportReason('Незаконные товары и услуги')}
              >
                Незаконные товары и услуги
              </button>
              <button
                className={reportReason === 'Порнографические материалы' ? 'active' : ''}
                onClick={() => setReportReason('Порнографические материалы')}
              >
                Порнографические материалы
              </button>
              <button
                className={reportReason === 'Персональные данные' ? 'active' : ''}
                onClick={() => setReportReason('Персональные данные')}
              >
                Персональные данные
              </button>
              <button
                className={reportReason === 'Мошенничество' ? 'active' : ''}
                onClick={() => setReportReason('Мошенничество')}
              >
                Мошенничество
              </button>
              <button
                className={reportReason === 'Нарушение авторских прав' ? 'active' : ''}
                onClick={() => setReportReason('Нарушение авторских прав')}
              >
                Нарушение авторских прав
              </button>
              <button
                className={reportReason === 'Спам' ? 'active' : ''}
                onClick={() => setReportReason('Спам')}
              >
                Спам
              </button>
              <button
                className={reportReason === 'Другое' ? 'active' : ''}
                onClick={() => setReportReason('Другое')}
              >
                Другое
              </button>
              <button
                className={reportReason === 'Не нарушает закон, но надо удалить' ? 'active' : ''}
                onClick={() => setReportReason('Не нарушает закон, но надо удалить')}
              >
            ...
            {reportReason === 'Другое' && (
              <div className="report-description">
                <label>Опишите, что нарушает данное объявление?</label>
                <textarea
                  value={reportDescription}
                  onChange={(e) => setReportDescription(e.target.value)}
                  minLength={10}
                  maxLength={500}
                  placeholder="Минимум 10 символов, максимум 500"
                />
                <span className="char-counter">{reportDescription.length}/500</span>
              </div>
            )}
            <div className="modal-actions">
              ...
              <button className="btn-cancel" onClick={handleCloseReport}>
                Отмена
              </button>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={handleCloseRental}>
          <div className="modal-content rental-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="rental-calculator">
              <label>
                Введите количество часов на которое готовы взять в аренду
