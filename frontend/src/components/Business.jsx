...
const LISTING_TYPES = ['Продать', 'Купить', 'Сдать в аренду', 'Поиск Заместителя'];
const PRICE_CONFIG = {
  'Продать': {
...
function Business() {
  ...
  useEffect(() => {
    document.title = 'Arz Store | Бизнес';
  }, []);
  ...
  const fetchAds = async (currentOffset) => {
    ...
    try {
      let url = `http://localhost:8080/api/ads?category=business&server=${selectedServer}&limit=20&offset=${currentOffset}`;
  ...
  const fetchAdCount = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/getadcount?CategoryName=business');
      const data = await response.json();
  ...
  const handleCreateListing = async () => {
    ...
    const imagePath = `ads/business/${timestamp}_${user?.nickname || 'user'}.${fileExtension}`;
    ...
    formDataToSend.append('category', 'business');
          ...
          <div className="listing-form">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={handleInputChange}
                maxLength={25}
                placeholder="Например: АЗС со скидкой"
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={handleInputChange}
                maxLength={500}
                placeholder="Опишите бизнес подробнее..."
    ...
    <div className="mobile-category-page">
      ...
      <div className="mobile-category-content">
        <div className="mobile-category-header">
          ...
          <h1 className="mobile-category-title">Бизнесы</h1>
        </div>
